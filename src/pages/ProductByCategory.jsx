...
function CategoryProducts() {
    ...
    async function getProductDatas() {
        const response = await fetch('http://localhost:5000/api/products/category/'+id+'?page=' + page,{
            headers: {
                ...
                <h3 className="text-center my-4">Products Lists</h3>
                {
                    localStorage.getItem('role') != 1 ?
                        <div className="row ml-4">
                            <button className="btn btn-success w-25" onClick={createCategory}>+Add new Products</button>
                        </div>
                    : ``
                }
            </div>
                            ...
                            datas.map((data) => {
                                return (
                                    <div className="card mx-2 my-3" style={{ width: 18 + 'rem', padding:0 }} key={data._id}>
                                        ...
                                        {
                                            localStorage.getItem('role') != 1 ? <div>
